...
//    def receive(future: Future[P2PResponse]): Int //type erasure, not possible to compile
    // lots of overloads
  }
  ...
  val addFV = add1 _ // return type is always Int
  println(addFV(1))
  println(addFV("3"))

  val receiveFV = receive _
// receiveFV(new P2PResponse) // compiler can't compile becase return type is unknown
